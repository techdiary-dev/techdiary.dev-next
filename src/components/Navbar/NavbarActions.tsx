...
const NavbarActions: React.FC<NavbarActionsProps> = ({ authSession }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {authSession ? (
        <AuthenticatedMenu authSession={authSession} />
      ) : (
        <UnAuthenticatedMenu authSession={authSession} />
      )}
    </div>
  );
};
...
const AuthenticatedMenu = ({ authSession }: { authSession: IAuthSession }) => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger>
        <Avatar>
          <AvatarImage src={authSession?.user?.image ?? ""} />
